...
- **`sources` には記事の実URLを入れる。** 候補の `url` は
  `https://news.google.com/rss/articles/...` というGoogleニュースの
  中間URLになっている。そのまま入れるとアプリの「出典を見る」が
  Googleニュースに飛んでしまうので、URL先を開いたときに判明する
  実際の媒体のURL（`https://prtimes.jp/...` など）を記録すること。
  住所・期間の確認でどのみち開く必要があるので、開いた先のURLを使えばよい。
